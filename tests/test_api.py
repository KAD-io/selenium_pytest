...
@pytest.mark.parametrize("first_name,last_name,company_id", BODY.USER_WITH_NOT_ACTIVE_COMPANY)
def test_code_create_negative_user(create_user, first_name, last_name, company_id):
    assert create_user.status_code == 400


@pytest.mark.parametrize("first_name,last_name,company_id", BODY.USER_WITH_NOT_ACTIVE_COMPANY)
def test_response_create_negative_user(create_user, first_name, last_name, company_id):
    response = create_user.json()
    assert str(response['detail']['reason']).find('Because it is not active') != -1


@pytest.mark.parametrize("first_name,last_name,company_id", BODY.USER_WITH_ACTIVE_COMPANY)
def test_code_create_user(create_user, first_name, last_name, company_id):
    assert create_user.status_code == 201


@pytest.mark.parametrize("first_name,last_name,company_id", BODY.USER_WITH_ACTIVE_COMPANY)
def test_response_create_user(create_user, first_name, last_name, company_id):
    response = create_user.json()
...
def test_code_valid_auth(authorize, login, password):
    assert authorize.status_code == 200


@pytest.mark.parametrize("login, password", BODY.VALID_AUTH)
def test_response_valid_auth(authorize, login, password):
    assert authorize.json()['token']


@pytest.mark.parametrize("login, password", BODY.INVALID_AUTH_LOGIN)
def test_code_invalid_login_auth(authorize, login, password):
    assert authorize.status_code == 422


@pytest.mark.parametrize("login, password", BODY.INVALID_AUTH_LOGIN)
def test_response_invalid_login_auth(authorize, login, password):
    assert authorize.json()['detail'][0]['msg'] == MSG.INVALID_LOGIN


@pytest.mark.parametrize("login, password", BODY.INVALID_AUTH_PASS)
def test_code_invalid_pass_auth(authorize, login, password):
    assert authorize.status_code == 403


@pytest.mark.parametrize("login, password", BODY.INVALID_AUTH_PASS)
def test_response_invalid_pass_auth(authorize, login, password):
